...
bool CameraDevice::set_save_info() const
{
   ...
   std::this_thread::sleep_for(35ms);
    ...
    text path = "/home/nvidia/wjd/huaian/CrSDK_back2/images";
    // auto save_status = SDK::SetSaveInfo(m_device_handle
    //    , const_cast<text_char*>(path.data()), TEXT(""), ImageSaveAutoStartNo);

        auto save_status = SDK::SetSaveInfo(m_device_handle
       , const_cast<text_char*>(path.data()), TEXT("ABCDE"), ImageSaveAutoStartNo);
...
void CameraDevice::OnCompleteDownload(CrChar* filename, CrInt32u type )
{
    if(!ofs.is_open()){
        ...
        std::string currenttime =std::to_string(ltm->tm_mon+1) +"_" +std::to_string(ltm->tm_mday)+"_"+ std::to_string(ltm->tm_hour)+"_"+std::to_string(ltm->tm_min)+"_"+std::to_string(ltm->tm_sec);
        ofs.open("/home/nvidia/wjd/huaian/CrSDK_back2/csvpath/"+currenttime+".csv");
    ...
    case SCRSDK::CrDownloadSettingFileType_None:
    {
        ...
        currentimg_name = file.data();
        std::filesystem::path name_path(currentimg_name);
        
        std::string img_name = name_path.filename().string();
...

        {
            ...
            gpsqua.alt = gpsinfo.alt;
            gpsqua.lat = gpsinfo.lat;
            gpsqua.lon = gpsinfo.lon;
            gpsinfo.relative_alt = gpsinfo.relative_alt;            
        }
   ...
   
        {
            ...
            gpsqua.roll =eulerAngle(2)*180.0/M_PI;
            gpsqua.yaw = eulerAngle(0)*180.0/M_PI;
            gpsqua.pitch = eulerAngle(1)*180.0/M_PI;

        }

        double res_lon = static_cast<double>(gpsqua.lon)/ static_cast<double>(1e7);
        std::stringstream ss_lon;
	    ss_lon<<std::setprecision(15)<<res_lon;
        std::string str_lon = ss_lon.str();

        double res_lat = static_cast<double>(gpsqua.lat)/ static_cast<double>(1e7);
        std::stringstream ss_lat;
        ss_lat<<std::setprecision(15)<<res_lat;
        std::string str_lat = ss_lat.str();
       
        
        double res_alt = static_cast<double>(gpsqua.alt)/ static_cast<double>(1e3);
        std::stringstream ss_alt;
        ss_alt<<std::setprecision(15)<<res_alt;
        std::string str_alt = ss_alt.str();
       ...
       ofs.flush();
        ...
        ofs<<str_alt<<",";
                ofs.flush();
        ofs<<std::to_string(gpsqua.yaw)<<",";
                ofs.flush();
        ofs<<std::to_string(gpsqua.roll)<<",";
                ofs.flush();
        ofs<<std::to_string(gpsqua.pitch)<<"\n";
...
//             tout << "code =  " << prop.GetCode()<<"\n";
//             switch (prop.GetCode()) {
//                 case SDK::CrDeviceProperty_ImageID_Num:
//                     auto value= prop.GetCurrentValue();
...
//                 break;
//                 // case SDK::CrDeviceProperty_ImageID_String:
//                 //     auto aaa= prop.GetCurrentValue();
//                 //     tout<<"value="<<aaa<<"\n";
//                 // break;
//                 // default:
//                 // break;
...
void CameraDevice::set_destination(){

    std::this_thread::sleep_for(100ms);
        SDK::CrDeviceProperty prop;
        prop.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_StillImageStoreDestination);

        prop.SetCurrentValue(SDK::CrStillImageStoreDestination::CrStillImageStoreDestination_HostPCAndMemoryCard);
       // prop.SetCurrentValue(SDK::CrStillImageStoreDestination::CrStillImageStoreDestination_HostPC);


        prop.SetValueType(SDK::CrDataType::CrDataType_UInt16Array);

        auto result = SDK::SetDeviceProperty(m_device_handle, &prop);
     if (CR_FAILED(result)) {
            tout << "failed changedir.\n";
        }else if(CR_SUCCEEDED(result)){
            tout << "successed changedir.\n";
        }
    }

void CameraDevice::set_num_setting(){
        SDK::CrDeviceProperty prop;
        prop.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_ImageID_Num_Setting);
        prop.SetCurrentValue(SDK::CrImageIDNumSetting::CrImageIDNumSetting_On);
        prop.SetValueType(SDK::CrDataType::CrDataType_UInt8Array);
        auto result = SDK::SetDeviceProperty(m_device_handle, &prop);
...
void CameraDevice::set_gpsinfo(){
        ...
        // SDK::CrDeviceProperty prop2;
        // prop2.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_ImageID_String);
        // prop2.SetCurrentValue((CrInt64u)"probhaha");
        // prop2.SetValueType(SDK::CrDataType::CrDataType_STR);
        // auto result2 = SDK::SetDeviceProperty(m_device_handle, &prop2);


        
        SDK::CrDeviceProperty prop3;
        prop3.SetCode(SDK::CrDevicePropertyCode::CrDeviceProperty_ImageID_Num);
        CrInt64u  sss = 0x2024;

        prop3.SetCurrentValue(sss);
        prop3.SetValueType(SDK::CrDataType::CrDataType_UInt64Range);
        auto result3 = SDK::SetDeviceProperty(m_device_handle, &prop3);
...
void CameraDevice::SerialThread(){
	...
	try {
        ...
        // time_t now = time(0);
        // tm * ltm = localtime(&now);
        // std::string currenttime = std::to_string(ltm->tm_hour)+"_"+std::to_string(ltm->tm_min)+"_"+std::to_string(ltm->tm_sec);
        // ofs.open("/home/ubuntu/wjd/CrSDK_back2/csvpath/"+currenttime+".csv");
		...
		try {
			...
			if (mavlink_parse_char(chan, serialData, &msg_mav, &status)) {
				...
				case MAVLINK_MSG_ID_GIMBAL_DEVICE_ATTITUDE_STATUS:
				{
                    ...
                    mavlink_msg_gimbal_device_attitude_status_decode(&msg_mav,&gimbal_device_attitude_status);
...
EulerAngles CameraDevice::ToEulerAngles(Quater q){
    EulerAngles angles;
